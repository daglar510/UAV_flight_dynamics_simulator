...
def pulse_input_row(
    ...
    return rx.el.div(
        rx.el.input(
            ...
            on_change=lambda val: FlightSimState.update_pulse_param(
                index, "start_time", val
            ),
            debounce_timeout=500,
            class_name="w-1/3 px-2 py-1 border rounded-md text-sm",
        ),
        rx.el.input(
            type="number",
            step="any",
        ...
        rx.el.button(
            rx.icon(
                tag="circle_minus", class_name="h-4 w-4"
            ),
            on_click=lambda: FlightSimState.remove_pulse(
                index
            ),
